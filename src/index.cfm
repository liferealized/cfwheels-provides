...
<p>Add a call to the display() function at the bottom of any action that you want to return XML or JSON.</p>
<blockquote><p>&lt;cfset display(yourModelObject)&gt;</p></blockquote>
...
<li><strong>display(object=dataToDisplay)</strong> - Call display at the bottom of every action that you want to respond to different formats. The argument object should be passed the data to transform to XML or JSON if that format is requested. Please note that you can also pass display any arguments for renderPage() and they will be passed through appropriately.</li>
</ul>
<p>The display() method should be able to take lists (strings), arrays, structures, arrays of structures, queries and arrays of wheels model objects and properly display them as XML or JSON. If the request type is set to HTML, the data is ignored and renderPage() is called.</p>
